#!/usr/bin/env bash
#
# Consilium deployment kit: restore. See deploy/kit.py for what it does, and
# docs/RUNBOOK.md for when to run it.
#
#   ./restore.sh --config /etc/consilium/consilium.conf --from <backup dir> [--yes | --as-site NAME --db-name DB]
#
