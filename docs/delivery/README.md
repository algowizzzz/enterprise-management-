...
| `Consilium-Leadership-Briefing.pptx` | 49-slide leadership briefing: what was built (including the redesigned portal and the Integrations screen), the evidence, AI connected and tested, what is not done, the road to production and the decisions asked for. Screenshots come from the final onboarding guide | `.venv/bin/python scripts/deck/build_deck.py`: Python (python-pptx), sources in `scripts/deck/` (`facts.json` for every number and status, `slides.py` for the slides), screenshots from `docs/guides/images/`. See `docs/OPERATIONS.md` §F |
