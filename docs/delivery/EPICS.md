...
working tree, the demonstration site and the latest full test run on a clean
site: **1453 tests, OK** (0 failures, 0 errors). Other runs:

- `pytest tests/`: 57 passed.
- Interface sweep: 302 passed, 0 failed.
...
**Where the Partial stories cluster.** Of the 40 stories not done:
...
* **Small product gaps:**
  ...
  - Automated checks for the My work views, list export and layout polish
    (E35-S3…S5).

**Being built now (18 Sep):** the eight class (d) requirement clauses (G-8,
G-10, G-14, O-4, P-8, E-8, E-9, O-3; see `REQUIREMENTS-COVERAGE.md` §4.1). The
stories they touch will be re-measured when that round finishes.
...
| *Phase 10 — Navigation, integrations and hand-offs* | | | | |
| E35 Navigation, search and interface polish | 2 | 3 | 0 | 5 |
| E36 Integrations and external hand-offs | 6 | 0 | 0 | 6 |
| **Phase 10 total** | **8** | **3** | **0** | **11** |
| **Original backlog (E1–E26)** | **100** | **27** | **0** | **127** |
| **All stories** | **151** | **39** | **1** | **191** |
...
| ID | Story | Size | Status |
|---|---|---|---|
| E22-S1 | As an engineer, the application can call the external AI service. **AC:** one adapter; endpoint and credentials are configuration; failure degrades gracefully and never blocks the user's work. | M | ✅ Done. `consilium_core/ai/client.py` is the one path for the help assistant and the analysis features; endpoint, model and a write-only key set in Admin → Integrations (`Assistant Settings`; request format "OpenAI-compatible API"), off by default, with *Test connection*; timeout, error, refusal or unreachable endpoint falls back to the built-in result. Exercised live on the demo site against an OpenAI-compatible provider: 20 logged requests, all succeeded. consilium_core.tests.test_ai_features.TestClient (all 3), consilium_core.tests.test_assistant.TestAIMode (all 8), TestSettings (all 4), consilium_core.tests.test_integrations.TestIntegrationsPage (all 14). |
...
| ID | Story | Size | Status |
|---|---|---|---|
| E29-S1 | As a recipient, I receive notifications by email. **AC:** an email adapter over the framework's mail queue; the channel reports itself unavailable with no outgoing account; a refused recipient is recorded as suppressed; failures retry. | M | ✅ Done. `email` adapter over the framework mail queue, or over Microsoft Graph (E36-S5); unavailable with no outgoing account (falls back to RECORD); opted-out or disabled recipients recorded as suppressed; hourly `notification.retry_failed`. consilium_core.tests.test_notification_templates.TestEventApi (all 10), TestEmailRetry (all 3). Real SMTP or a real Graph tenant is unexercised (needs the target) |
...
| E31-S4 | As a user, I ask the portal how to do something here. **AC:** built-in offline answers per page and permission; never reveals a record I cannot read; every question logged append-only with an hourly limit; an optional AI endpoint, off by default, falls back to the built-in answer. | L | ✅ Done. `consilium_core/assistant/`: built-in offline answers per page and permission, drawn from the page map, the glossary and the illustrated guide chapters (administration chapters for administrators only), with citations; append-only question log with an hourly limit; optional endpoint off by default with fallback. The AI prompt treats access findings as authoritative only for the actions they name. Answers with citations through an OpenAI-compatible provider on the demo site. consilium_core.tests.test_assistant (TestBuiltInAnswers (all 12), TestRecordAwareness (all 6), TestAdminDocumentation (all 2), TestAuditAndLimits (all 4), TestSettings (all 4), TestAIMode (all 8)); browser journey `help-assistant` |
...
| E32-S5 | As a user or administrator, I have a guide. **AC:** task-based guides for both audiences, matching the screens. | M | ✅ Done. The onboarding guide in `docs/guides/`: chapters 00–10 and a glossary, task-based and written for business users, with chapter 8 by role and chapter 9 on administration without code. Every screen is shown with numbered callouts. It is built by `scripts/build_guides.sh` into `Consilium-Onboarding-Guide` (PDF and DOCX, 217 pages, 185 screenshots) outside the repository. `docs/USER-GUIDE.md` and `docs/ADMIN-GUIDE.md` are the quick starts, and the technical material is in `docs/OPERATIONS.md`; shipped to the help assistant by the bundle |
...
| E34-S5 | As an administrator, people sign in through the corporate directory. **AC:** the protocol is decided (LDAP and OIDC are configuration; SAML is a separate build); existing users matched without re-keying; closes E2-S4. | L | 🟡 Partial. OIDC proven end to end against a mock provider on the stand-in, existing user matched without re-keying (E2-S4); LDAP is configuration (`[sso] mode = ldap`, pure-Python `ldap3`), not exercised. Admin → Integrations shows the redirect URI to register (E36-S6). **No corporate identity provider or directory connected; protocol choice, redirect-URI registration and claims need the target** |
...
Built in the last round before release, after the product owner's review of
the portal. None of it was in the original backlog.

## E35 — Navigation, search and interface polish
*Outcome: people find their way, and find a record, without knowing which
inventory it lives in.* Trace: stakeholder directives, O-1, G-2, P-2, E-2.

| ID | Story | Size | Status |
|---|---|---|---|
| E35-S1 | As a user, the header shows the areas I can use as menus of their pages. **AC:** a two-row header (brand, search, text size, theme, user menu); menus Home, My work (with a count badge), Governance, Policies, Escalations, Insights and Admin; each menu a panel of sub-pages with a one-line description; a menu or item shown only to those who may use it; usable by keyboard; a slide-out menu on phones. | M | ✅ Done. `consilium_core/navigation.py` (`cns_nav`, permission and role checks per item) and `templates/base_portal.html`. consilium_core.tests.test_navigation.TestNavigation (all 11); browser journeys `menu-keyboard`, `phone-menu`, `non-admin-no-admin-nav`, `inbox-badge` (11/11) |
| E35-S2 | As a user, I find a forum, policy or escalation from any page. **AC:** one search box ("/" or Ctrl/Cmd+K); results grouped by record type, each opening its record; confidential documents and sensitive matters never shown to those not cleared; "See all" opens the list with the words carried over. | M | ✅ Done. `consilium_core/search.py` (GET only, every row through `frappe.get_list`); `/forums` and `/escalations` accept `?q=`. consilium_core.tests.test_global_search.TestGlobalSearch (all 9); browser journey `global-search` |
| E35-S3 | As a user, My work opens straight onto the kind of task I want. **AC:** views for approvals, reviews, attestations and escalations waiting for an owner, reachable from the menu. | S | 🟡 Partial. `/tasks?show=approvals`, `reviews`, `attestations`, `unowned` and `soon`, linked from the My work menu; the list behind them is tested (consilium_core.tests.test_inbox (all 22)). **The view filter runs in the browser, and no automated test selects a view** |
| E35-S4 | As a user, I export the list I am looking at. **AC:** Export CSV on the forum, policy, escalation and formation-request lists; the rows matching the filters and search, through the permission-checked list; capped; safe to open in a spreadsheet. | S | 🟡 Partial. `consilium-table.js` exports up to 5,000 rows through the list API and neutralises spreadsheet formulas. **No automated test downloads an export** |
| E35-S5 | As a user, every page looks and behaves the same way. **AC:** consistent page headers and actions; empty states that say what to do next; visible focus rings; no visible layout shift as a page loads. | M | 🟡 Partial. The shared page header, empty-state and focus styles are in `public/css/consilium.css` and used by every page; every page renders in the 302-check interface sweep; the keyboard walk checks focus. Layout shift was measured under 0.06 when the polish was done. A later pass fixed twelve interface issues found in review. It made the naming consistent (Policy library, Insights, Gaps and risk, Administration), described My work approvals in words, and counted the time-limit clocks only over records the viewer may read. It also let originators answer a returned request, opened the policy change log to the policy office, offered "Advanced view" to administrators only, and labelled times with their zone. **No automated check of empty states or layout shift** |

## E36 — Integrations and external hand-offs
*Outcome: an administrator connects the organisation's own tools from one
screen, and nothing about them is hard-coded.* Trace: O-6, O-7, G-14, P-14,
E-17, D-1.

| ID | Story | Size | Status |
|---|---|---|---|
| E36-S1 | As an administrator, I connect the platform's integrations from one screen. **AC:** Admin → Integrations, administrators only; one card per integration; a secret can be replaced but is never shown back; only a card's own fields are written. | M | ✅ Done. `/integrations`; `consilium_core/integrations/admin.py` (`state`, `save`, POST only). Settings in `Assistant Settings`, `External Tools Settings` and `Email Delivery Settings`; each single settings record hides the desk sidebar. consilium_core.tests.test_integrations.TestIntegrationsPage (all 14) |
| E36-S2 | As an administrator, I know the AI connection works before anyone relies on it. **AC:** *Test connection* goes through the audited client and reports success and latency or the error; a warning when an AI switch is on and no key is saved; the request format is named for what it is. | S | ✅ Done. `admin.test_ai_connection`; request format renamed to "OpenAI-compatible API", with a patch for existing sites (`patches/v0_1/rename_openai_request_format`). The demo site is connected to an OpenAI-compatible provider. TestIntegrationsPage (connection-test and legacy-format tests) |
| E36-S3 | As a policy author, I open a governing document in the organisation's document editor. **AC:** "Open in Doc AI" on the policy page, each version row and the document viewer; access and handling checked, and the hand-off logged, before the browser leaves; only identifiers in the address, never the file; a clear "not connected yet" explanation until an administrator sets the address. | M | ✅ Done. `/doc-ai` and `external_tools.doc_ai_handoff` (Access Log row "Doc AI hand-off"; refusals audited; addresses checked when saved and again when used). consilium_core.tests.test_integrations.TestDocAi (all 9), TestAddresses (all 5) |
| E36-S4 | As a policy owner, I go to the organisation's horizon-scanning platform from where I work. **AC:** a button on the policy register and on regulatory updates; the label is configuration; it explains itself when not connected. | S | ✅ Done. `/horizon-scanning`, `external_tools.horizon_target`; the menu item follows the settings. TestHorizonScanning (all 3), consilium_core.tests.test_navigation.TestNavigation.test_horizon_scanning_follows_the_integration_settings |
| E36-S5 | As a deploying organisation that allows no SMTP from servers, I send notifications through Microsoft Graph. **AC:** app-only client credentials, no redirect URL; the token cached; a revoked token renewed once; throttling waited out per Retry-After; a timeout never re-sent at once; the same dispatch record and hourly retry as SMTP; configurable from the deployment configuration and Admin → Integrations; a test e-mail to the administrator. | M | ✅ Done. `consilium_core/integrations/graph_mail.py` (standard library only); `graph_*` keys in `consilium.conf`, applied by `deploy/configure_site.py`; *Send a test email to me*. consilium_core.tests.test_graph_mail (TestTheWire (all 8), TestTheDispatchRecord (all 7), TestTheSettings (all 5)), TestIntegrationsPage.test_a_test_email_through_graph_reaches_the_administrator, tests/test_kit.py Graph tests. A real tenant is unexercised (needs the target). The framework's own mail (password reset) still needs SMTP |
| E36-S6 | As an administrator, I see what to register with the identity provider. **AC:** the single sign-on card shows the mode and the redirect URI with a copy button; switching SSO stays in the deployment configuration. | S | ✅ Done. `admin.sso_card`. TestIntegrationsPage.test_the_single_sign_on_card_shows_the_redirect_address |
...
- Real certificates, the identity provider, the mail route (an SMTP relay or a
  Microsoft Graph tenant) and load testing need the target environment.
